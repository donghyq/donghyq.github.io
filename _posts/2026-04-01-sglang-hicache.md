...
GPU 层的物理存储由 `MHATokenToKVPool` 管理。核心是两个预分配的大 Tensor，所有请求的 KV Cache 共享这一块连续显存：

```python
# k_buffer 和 v_buffer 的形状：
# [num_layers, pool_capacity, page_size, num_kv_heads, head_dim]
# pool_capacity = slot 总数（即 page 总数）
# page_size = 每个 slot 容纳的 token 数（默认 16）
k_buffer = torch.zeros(num_layers, pool_capacity, page_size, num_kv_heads, head_dim, 
                       dtype=dtype, device="cuda")
v_buffer = torch.zeros(num_layers, pool_capacity, page_size, num_kv_heads, head_dim, 
                       dtype=dtype, device="cuda")
```

`pool_capacity` 由显存预算决定：

```
每个 page 占用 = page_size × num_kv_heads × head_dim × dtype_bytes × 2(K+V) × num_layers
pool_capacity = KV Cache 显存预算 ÷ 每个 page 占用
# 例：32层, 8 KV head, 128维, float16, page_size=16 → 每 page 约 16MB
# 分配 40GB → pool_capacity ≈ 2500 个 slot
```

当 TreeNode 的 `value = [5, 8]` 时，表示这个节点拥有 2 个 page（共 32 个 token 的 KV）。Attention 计算时通过数组下标直接索引：

```python
# 取某一层的 K 数据：
K = k_buffer[layer_i, [5, 8], :, :, :]  # 形状 [2, 16, 8, 128] → 展平为 [32, 8, 128]
```

同一组 slot 编号贯穿所有 32 层——分配一次，每层都用同一个下标。没有 hash 计算、没有查表，就是纯粹的数组索引，O(1) 定位。
...
### 5.4 为什么 L1/L2 用 slot 索引，L3 用内容寻址

两种寻址方式的选择由各层面对的核心矛盾决定：

**L1/L2 选择 slot 索引的原因：**

- **读取频率极高**：Forward 每一层都要读 KV Cache 做 Attention，一次 forward（32 层）就是 32 次读取。slot 索引是纯数组下标，O(1) 零开销；如果用 hash 寻址，32 层就多 32 次 SHA256 计算 + hash table 查找，在 GPU 热路径上不可接受。
- **支持 in-place 更新**：Decode 阶段每生成一个 token 就往 slot 里追加写入，不需要重新计算 hash。
- **单机连续地址空间**：GPU/Host 内存是一块预分配的连续 Tensor，天然有全局统一编号。

**L3 选择内容寻址的原因：**

- **天然去重**：相同 system prompt 产生的 KV Cache 内容相同，hash 相同，只存一份。L3 数据量最大，去重收益最高。
- **适配分布式**：L3 可能跨多台机器（Mooncake RDMA 内存池），没有全局统一的 slot 编号空间。内容寻址是"自描述"的——知道内容就能算出 key，任何节点都能独立定位，不依赖中心化分配器。
- **IO 延迟容忍**：L3 读写本身就慢（SSD 毫秒级、RDMA 几十微秒级），多一次 hash 计算（微秒级）相比 IO 开销可忽略。

hash 本身存储在 TreeNode 的 `hash_value` 字段中。Radix Tree 是每个推理节点本地维护的内存数据结构，当 page 被 backup 到 L3 时算好 hash 并记录；prefetch 时拿着 hash 去 Storage backend 查询。
