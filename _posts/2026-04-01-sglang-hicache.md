...
关键在于：搬运是异步的、流水线化的。HiCache 用独立的 CUDA Stream 做 GPU-Host DMA 拷贝，用独立的 Python 线程做 Host-Storage IO，并且和前向计算在时间上重叠。读者需要记住的核心等式是：**搬运延迟 ≤ 计算延迟 + 排队延迟**——只要这个不等式成立，用户感知不到任何性能损失。
...
### 单体 vs PD 分离：同一套四阶段，两种物理拓扑

上述四阶段在 PD 分离架构下**逻辑完全不变**，但物理路径发生了分裂：
...
关键洞察：SGLang 没有为 PD 分离重写传输系统。Prefill 端的 `write_backup → backup_thread` 就是"发送"，Decode 端的 `prefetch_from_storage` 就是"接收"——HiCache 的三层架构天然适配了跨机数据总线的角色。具体的 RDMA 零拷贝细节和 ZMQ 事件广播机制将在 Part IV（Chapter 09）展开。
...
> 记忆锚点：TreeNode 是索引卡片，三个字典（`ongoing_write_through` / `ongoing_load_back` / `ongoing_prefetch`）是异步任务的生命周期追踪器。
...
当 TreeNode 的 `value = [5, 6, 7]` 时，表示这个节点的 3 个 page 占据了 Pool 中第 5、6、7 行（每行存一个 page 即 16 个 token 的 KV）。模型做 Attention 时，通过 `k_buffer[:, indices, :, :]` 直接取出连续的 KV 数据。

### 5.2 Host Pool（L2）

Host 层由 `MHATokenToKVPoolHost` 管理，结构与 GPU Pool 完全对齐——相同的 shape、相同的 page_size，区别在于内存类型是 **Pinned Memory**（锁页内存）。锁页内存保证 CUDA DMA 拷贝时物理地址不变（不会被 OS 换出到 swap），让 `cudaMemcpyAsync` 能高效运行。
...
当 TreeNode 的 `host_value = [12, 13, 14]` 时，含义和 GPU 侧完全对称：该节点的 3 个 page 存在 Host Pool 的第 12、13、14 slot 中。
...
天然去重——如果两个不同请求产生了完全相同内容的 page（比如相同 system prompt 的相同位置），hash 一样，Storage 中只存一份。Backend 可以是本地 SSD（通过 hf3fs）、也可以是分布式共享内存池（Mooncake、Nixl）——在 PD 分离架构下后者尤为关键。
...
Write-Through Selective 的精巧之处：第一次命中的数据可能是冷数据（只有一个请求用过），不值得占用 Host 内存去备份；当同一个前缀被第二次命中时，说明它是热门数据，值得花 DMA 带宽去备份。
...
Case 1 最高效：由于之前 Write-Through 已经完成备份，驱逐只需修改元数据（`value` 设为 None），零等待。Case 2 最危险——不得不在热路径上阻塞等 DMA。Case 3 最决绝——对于冷数据（hit_count < 2），直接丢弃比备份更划算。
...
class LayerDoneCounter:
    ...
    def wait(self, layer_i):
        current_event.layer_events[layer_i].synchronize()
        # 同步完成后，该层的 KV 数据已经在 GPU 上，可以做 Attention
```
...
正常运行时只查询不等待，让 GPU 继续计算；只有 write_back evict 的紧急情况下才阻塞——这就是 Write-Back 策略在延迟上的隐含成本。
...
> Storage IO 由三个独立 Python 后台线程驱动，线程间通过 Queue 解耦——主线程只负责入队和处理 ACK，不做任何 IO 等待。
...
在单体架构下，L3 是一个可选的"冷备层"——有它更好，没它也能跑。但在 PD 分离架构下，L3 成为了唯一的跨节点数据传输通道：Prefill 节点计算完的 KV Cache 必须经过 L3（通常是 Mooncake 或 Nixl 提供的分布式 RDMA 内存池）传递给 Decode 节点。
...
通过在最上层将流量导入不同队列，Prefill 和 Decode 节点的调度策略完全解耦。Prefill 节点关心的是"算完后尽快把 KV 写入全局池"，Decode 节点关心的是"等 KV 到位后尽快开始生成"。
...
由于 KV Cache 在 L3 中是基于内容 Hash 寻址的，一个有趣的副产品是：如果用户 B 的请求和用户 A 共享相同的 system prompt，即使它们被调度到不同的 Prefill 机器，产生的 KV Cache hash 相同——Decode 节点拿着 hash 去 L3 查询时直接命中，无需重复计算。这就是 Cross-Node Prefix Caching，Radix Tree 的前缀复用从单机扩展到了整个集群。
...
Encoder 完成计算后，RDMA 网卡直接将 Embedding 数据写入 Decoder 侧预注册的物理内存。本地只需一次 `.view(dtype).reshape(shape)`（纯元数据操作，零数据拷贝）就得到可用的 PyTorch Tensor。
...
> DeepSeek-V2/V3 引入的 Multi-head Latent Attention (MLA) 从根本上改变了 KV Cache 的形态。本章梳理 MLA 的核心原理，以及它在 HiCache 三层架构中引发的四个关键适配点。
...
单个 page 的体积缩小直接带来两个好处：Host Pool 能存更多 page（等效容量翻数十倍），L3 的 IO 时间也按比例缩短。
...
```python
# hiradix_cache.py 中的条件判断
if model_config.attention_arch == "MLA":
    self.backup_skip = True  # 非 rank 0 跳过 L3 写入
    config_suffix = ""       # Storage 中不区分 rank
else:
    self.backup_skip = False
    config_suffix = f"_tp{tp_rank}"
```
...
由于所有 rank 的 latent 向量相同，它们计算出的 content hash 也相同。在 L3 中只需存一份（而非 tp_size 份）。对于 8-way TP 的 MHA 模型需要存 8 份不同 hash 的数据，而 MLA 只存 1 份——Storage 容量利用率提升 8 倍。
...
这是防御性设计：数据相同不意味着操作时序相同。如果 rank 0 的 DMA 已完成但 rank 3 还在进行中，提前解锁可能导致 rank 3 读到脏数据。

---

*SGLang HiCache 源码精读 | Generated by CatDesk*
